...
@section('header', 'Gestion des Unités de Mesure')

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="#">Accueil</a></li>
    <li class="breadcrumb-item active" aria-current="page">Référentiel</li>
    <li class="breadcrumb-item active" aria-current="page">Unités de Mesure</li>
@endsection
...
<div class="card">
    ...
    <div class="card-body">
        <div id="toolbar">
            @can('cores.referentiel.unites.store')
        ...
        <table id="table"
               data-toggle="table"
               data-url="{{ route('cores.referentiel.unites.data') }}"
            ...
            <thead>
                <tr>
                    ...
                    <th data-field="abreviation" data-sortable="true">Abréviation</th>
...
@include('core::referentiel.unites._modal')
@stop
...
<script>
    const routes = {
        store: "{{ route('cores.referentiel.unites.store') }}",
        update: "{{ route('cores.referentiel.unites.update', ':id') }}",
        destroy: "{{ route('cores.referentiel.unites.destroy', ':id') }}"
    };
    ...
    $('#btn-add').click(function() {
        $('#form')[0].reset();
        $('#itemId').val('');
        $('#modalLabel').text('Nouvelle Unité');
        $('#modal').modal('show');
    });
    ...
    function editItem(row) {
        $('#form')[0].reset();
        $('#itemId').val(row.id);
        $('#modalLabel').text('Modifier Unité');
        
        $('#libelle').val(row.libelle);
        $('#abreviation').val(row.abreviation);
        $('#type').val(row.type);
